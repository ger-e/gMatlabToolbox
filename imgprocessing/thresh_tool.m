...
try
	figpos = centrefig(55,65,0,0,0);
	h_fig = figure('pos',figpos);
catch
	h_fig = figure('units','normalized','pos',[0.1 0.15 0.8 0.65]);
end
...
if nargout>0 %return result(s)
  ...
  %Reposition so that GUI stays on screen when monitor resolution is low
%   if tmp(3)==1024
%       beep
%       disp('Monitor in low-res setting!')
%       set(h_fig,'pos',[225   458   560   420]);
%       drawnow;
%   end
