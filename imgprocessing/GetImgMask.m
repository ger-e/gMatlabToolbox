function [mask answer counter] = GetImgMask(img,counter,debug)
...
% % for manual use only!! -->take the largest n contiguous objects
clusters = bwlabel(mask);
stats = regionprops(clusters,'Area');
Areas = [stats.Area];
maskTemp = mask;
...
% % add in second largest
% Areas(indx) = 0;
% [SecondMaxArea indx] = max(Areas);
% mask2 = maskTemp;
% mask2(clusters~=indx) = 0;
% mask = mask + mask2;

% % add in third largets
% Areas(indx) = 0;
% [ThirdMaxArea indx] = max(Areas);
% mask3 = maskTemp;
% mask3(clusters~=indx) = 0;
...
% diskRadius = 15+250; % set size of disk to dilate by
diskRadius = 15;
