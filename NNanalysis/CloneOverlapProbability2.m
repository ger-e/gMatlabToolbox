...
% 07/16/2012: Fixed MAJOR bug: deleted code that removed matrix
% symmetry--this step is not necessary since we're already using 'unique'.
% It leads to an overestimation of the distances between cells. Also
% improved speed of distance measurements by processing vectors at a time
...
% MaskDir = 'C:\Users\Gerry\Desktop\New clone overlap probability\Masks\masks'; % path to DGmasks; PRECURSOR ONLY SIMULATION
MaskDir = 'C:\Users\Gerry\Desktop\New clone overlap probability\Masks\mask_strel265'; % path for ASTRO ONLY SIMULATION
...
if Astros
    ...
    ML_GCL_Hilus_MaskDir = 'C:\Users\Gerry\Desktop\New clone overlap probability\Masks\mask_strel265'; % path to masks that includes ML and hilus
    NumPrecursors = 8; % number of precursors to model
    NumAstros = 25; % number of astros to model
...
% for NumClones=2:10
    for b=1:NIterations
        ...
        for d=1:NumClones
            CloneDistances(:,d) = ((SampleCloneXYZ(d,1)-SampleCloneXYZ(:,1)).^2+(SampleCloneXYZ(d,2)-SampleCloneXYZ(:,2)).^2+(SampleCloneXYZ(d,3)-SampleCloneXYZ(:,3)).^2).^0.5;
        end
    ...
    ProbabilityDistribution = ProbabilityDistribution(ProbabilityDistribution~=0);
% end
