function Offsets = adjstartdelay_interlacing3b(ImgName,Directory,tempdir,Swap)
...
temp = imfinfo(fullfile(Directory,ImgName));
...
%     Img = imread(fullfile(Directory,ImgName),'Index',a);
    Img = ReadRawIJSlice(fullfile(Directory,ImgName),a);
...
%     Img = imread(fullfile(Directory,ImgName),'Index',b);
    Img = ReadRawIJSlice(fullfile(Directory,ImgName),b);
    Img = single(Img); % convert from uint16
    lace1 = Img(:,1:2:end);
    lace2 = Img(:,2:2:end);
    lace1 = permute(lace1,[2 1]);
