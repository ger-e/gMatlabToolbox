...
% 2/28/2012: re-visited the script to do further testing; added ability to
% explore the clustering output as a function of temperature
%
...
load spots.mat;
% Spots = [Spots(900:end,:)' Spots(1:200,:)']';
...
for ZZ=1:length(tempTest) % to allow you to explore many different temperatures
    tee = tempTest(ZZ);
    ...
    theta = .5; % 50, 100,150,200,250,300,350,400,450,500
    ...
    % Plot number of points in each cluster as a function of temperature
    NumPointsPerClus = zeros(1,size(FinalClus,3));
    for a=1:size(FinalClus,3)
        NumPointsPerClus(a) = sum(FinalClus(:,1,a) > 0); % find number of points in each cluster
    end

    % now plot
    figure(2); hold on;
    x(1:length(NumPointsPerClus)) = tee;
    y = NumPointsPerClus;
    y2 = size(FinalClus,3);
    plot(x,y,'o');
    
    % also plot the total number of clusters for each temperature
    figure(5); hold on;
    plot(x,y2,'x');
    clear x y y2;

    % plot histogram of times points have changed with each other
    figure(4); hist(PrelimClusters(PrelimClusters(:)>0));
end

% plot your clusters
figure(3); plot3(Spots(:,1),Spots(:,2),Spots(:,3),'k.'); hold on;

for r=1:size(FinalClus,3)
    RandColor = 0;
    while sum(RandColor<0.1)
        RandColor = rand(1,3);
    end
    figure(3); plot3(FinalClus(:,1,r),FinalClus(:,2,r),FinalClus(:,3,r),'.','Color',RandColor,'MarkerSize',20); hold on;
end
grid on; box on;
